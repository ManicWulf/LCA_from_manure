...
def energy_produced(eff_methane, env_config):       #in kWh
    ...
    factor_operating_time = h_in_y * operating_time
    heat = eff_methane * tvm_kwh * get_chp_efficiency_heat(env_config)
    electricity = eff_methane * tvm_kwh * get_chp_efficiency_electricity(env_config)
...
def electricity_generated_sofc(env_config, biomethane):
    ...
    return biomethane * tvm_kwh * get_efficiency_sofc(env_config)
