...
def total_carbon(methane_pot, env_config):
    ...
    """
    densitity_methane = dfc.find_value_env_config("density_ch4", "value", env_config)   # converting ch4 from m3 to kg CH4
    molar_ratio_c_ch4 = dfc.find_value_env_config("molar_ratio_c_to_ch4", "value", env_config)  # converting kg CH4 to kg C
...
def calc_farm_methane_pot(manure_methane_dict, dw_odw_dict, methane_pot_dict):
    ...
    manure_methane_dict['methane_solid'] = manure_methane_dict['manure_solid'] * dw_odw_dict['dw_solid'] * dw_odw_dict['odw_solid'] * methane_pot_dict['methane_pot_solid']
    manure_methane_dict['methane_liquid'] = manure_methane_dict['manure_liquid'] * dw_odw_dict['dw_liquid'] * dw_odw_dict['odw_liquid'] * methane_pot_dict['methane_pot_liquid']
    manure_methane_dict['methane_straw'] = manure_methane_dict['manure_straw'] * dw_odw_dict['vs_straw'] * methane_pot_dict['methane_pot_straw']
...
def calc_manure_and_methane_cattle(animal, animal_data_dict, animal_config, methane_pot_dict, dw_odw_dict):
    ...
    # Check for NaN values in manure_methane_dict and set them to 0
    for key, value in manure_methane_dict.items():
        if value is None or isinstance(value, float) and np.isnan(value):
            manure_methane_dict[key] = 0
...
def methane_pre_storage_emissions(carbon_emissions_dict, methane_dict, env_config, additional_data_dict):
    """
    :param carbon_emissions_dict: Methane emissions dict
    :param methane_dict: Methane emissions in m3
    :param env_config:
    :param additional_data_dict:
    :return: Methane emissions in kg CH4
    """
    # methane pre-storage emissions by source (liquid, solid, straw), converting from m3 to kg C
    ...
    carbon_emissions_dict['methane_emissions_straw'] = storage.emissions_manure_ch4(additional_data_dict['pre_storage'],
                                                                                    carbon_emissions_dict[
                                                                                        'c_tot_straw'],
                                                                                    env_config)
    carbon_emissions_dict['methane_emissions_solid'] = carbon_to_methane(env_config, carbon_emissions_dict['methane_emissions_solid'])
    carbon_emissions_dict['methane_emissions_liquid'] = carbon_to_methane(env_config, carbon_emissions_dict[
        'methane_emissions_liquid'])
    carbon_emissions_dict['methane_emissions_straw'] = carbon_to_methane(env_config, carbon_emissions_dict[
        'methane_emissions_straw'])
...
def convert_ch4_emissions_to_volume(carbon_emissions_dict, env_config):
    ...
    list_names = ['methane_emissions_solid', 'methane_emissions_liquid', 'methane_emissions_straw', 'methane_emissions_pre_storage']
    for name in list_names:
        new_key = name + '_volume'
        carbon_emissions_dict[new_key] = methane_mass_to_volume(env_config, carbon_emissions_dict[name])
...
def calc_pre_storage_emissions_carbon(methane_dict, env_config, additional_data_dict):
    """
    :param methane_dict: methane potential in m3 ch4
    :param env_config:
    :param additional_data_dict:
    :return: methane emissions in m3
    """
    # pre storage emissions ch4 in kg C
    carbon_emissions_dict = {'c_tot': total_carbon(methane_dict['methane_tot'], env_config)}    #converting ch4 from m3 to kg C
    carbon_emissions_dict['methane_emissions_pre_storage'] = storage.emissions_manure_ch4(
        additional_data_dict['pre_storage'], carbon_emissions_dict['c_tot'], env_config)        # pre storage emissions in kg C
    carbon_emissions_dict['c_tot_pre_storage'] = carbon_emissions_dict['c_tot'] - carbon_emissions_dict[
        'methane_emissions_pre_storage']                                                        # CH4 in kg C left after pre storage emissions
    ...
    carbon_emissions_dict['methane_emissions_pre_storage'] = carbon_to_methane(env_config, carbon_emissions_dict[
        'methane_emissions_pre_storage'])                                                       # methane emissions prestorage in kg CH4

    # methane pre-storage emissions by source (liquid, solid, straw), in kg CH4
    carbon_emissions_dict = methane_pre_storage_emissions(carbon_emissions_dict, methane_dict, env_config,
                                                          additional_data_dict)

    """convert the emissions back to volume in m3, for solid, liquid, straw and total"""
...
def calc_farm_emissions_nitrogen(nitrogen_emissions_dict, env_config, additional_data_dict):
    """
    :param nitrogen_emissions_dict: n_tot in kg N
    :param env_config:
    :param additional_data_dict:
    :return: storage and field emissions in kg NH3 and kg N2O
    """
    ...
    nitrogen_emissions_dict['nh3_emissions_pre_storage'] = storage.emissions_manure_nh3(
        additional_data_dict['pre_storage'], nitrogen_emissions_dict['n_tot'], env_config)          # NH3 emissions in kg N
    ...
    )                                                                                                # NH3 emissions in kg N

    n_tot_new_untreated = nitrogen_emissions_dict['n_tot'] - nitrogen_emissions_dict[
        'nh3_emissions_post_storage_untreated']                                                      # N tot after NH3 storage emissions for untreated, in kg N
    n_tot_new = nitrogen_emissions_dict['n_tot'] - nitrogen_emissions_dict['nh3_emissions_pre_storage']     # N tot after NH3 storage emissions for treated, in kg N

    nitrogen_emissions_dict['n2o_emissions_pre_storage'] = storage.emissions_manure_n2o(
        additional_data_dict['pre_storage'], n_tot_new, env_config)                                     # N2O emissions in kg N, using n_tot_new

    nitrogen_emissions_dict['n2o_emissions_post_storage_untreated'] = storage.emissions_manure_n2o(
        additional_data_dict['post_storage'], n_tot_new_untreated, env_config)                          # N2O emissions in kg N for untreated, using n_tot_new_untreated
    ...
    nitrogen_emissions_dict['nh3_emissions_post_storage'] = storage.emissions_digestate_nh3(
        additional_data_dict['post_storage'],
        nitrogen_emissions_dict['n_tot_pre_storage'],
        env_config)                                                                                         # nh3 emissions post storage in kg N
    n_tot_new_post_storage = nitrogen_emissions_dict['n_tot_pre_storage'] - nitrogen_emissions_dict[
        'nh3_emissions_post_storage']                                                                       # N left in digestate after NH3 emissions post storage in kg N
    nitrogen_emissions_dict['n2o_emissions_post_storage'] = storage.emissions_digestate_n2o(
        additional_data_dict['post_storage'],
        n_tot_new_post_storage,
        env_config)                                                                                         # N2O emissions post storage in kg N, using Ntot after NH3 emissions
    nitrogen_emissions_dict['n_tot_post_storage'] = n_tot_new_post_storage - nitrogen_emissions_dict[
        'n2o_emissions_post_storage']                                                                       # N tot after storage emissions in KG N
    ...
    nitrogen_emissions_dict['nh3_emissions_field'], nitrogen_emissions_dict[
        ...
        env_config, nitrogen_emissions_dict['n_tot_post_storage'], application_method)                      # NH3 and N2O emissions field digestate in KG N
    ...
    nitrogen_emissions_dict['nh3_emissions_field_untreated'], nitrogen_emissions_dict[
        ...
        env_config, nitrogen_emissions_dict['n_tot_post_storage_untreated'], application_method)            # NH3 and N2o emissions field untreated in kg N
    ...
    nitrogen_emissions_dict['nh3_emissions_pre_storage'] = n_to_nh3(env_config, nitrogen_emissions_dict[
        'nh3_emissions_pre_storage'])                                                                       #convert kg N to kg NH3
    nitrogen_emissions_dict['n2o_emissions_pre_storage'] = n_to_n2o(env_config, nitrogen_emissions_dict[
        'n2o_emissions_pre_storage'])                                                                       # Convert kg N to kg N2O
    ...
    print(f'nitrogen emission dictionary, calc_farm_emissions_nitrogen function: {nitrogen_emissions_dict}')
...
def calc_gwp_impact(initial_data_dict, env_config):
    ...
    for key, value in initial_data_dict.items():
        ...
        if 'ch4' in key or 'methane' in key:
            ...
            print(f'CO2 eq emissions for {new_key} with value {value}: {env_impact_dict[f"co2_{new_key}"]}')
    ...
    """
    FOR BUG FIXING: make sure that CO2_N2O_tot is calculated correctly, by hand
    """
    print(f'Co2 N2o tot calculated directly: {env_impact_dict["co2_n2o_tot"]} \n untreated: {env_impact_dict["co2_n2o_tot_untreated"]}')
    env_impact_dict['co2_n2o_tot'] = env_impact_dict['co2_n2o_pre_storage'] + env_impact_dict['co2_n2o_post_storage'] + env_impact_dict['co2_n2o_field']
    env_impact_dict['co2_n2o_tot_untreated'] = env_impact_dict['co2_n2o_post_storage_untreated'] + env_impact_dict['co2_n2o_field_untreated']
    print(f'Co2 N2o tot calculated by adding pre, post and field together: {env_impact_dict["co2_n2o_tot"]} \n untreated: {env_impact_dict["co2_n2o_tot_untreated"]}')
...
def calc_single_farm(results_df, farm_data, animal_config, env_config):
    ...
    for animal in animal_types:
        ...
        if animal_data_dict['num_animal'] > 0:
            ...
            #print(f'Animal type: {animal} and stable type: {animal_data_dict["manure_type"]}')
            manure_type_in_animal_config = dfc.check_combination_exists_in_animal_config(animal, animal_data_dict["manure_type"], animal_config)
            #print(manure_type_in_animal_config)
            # check if manure type exists, if it doesn't it's probably a Pig or Poultry animal with a wrong input.
            if manure_type_in_animal_config:

                #print(f'calculating manure and methane for {animal} with first if statement. Manure type: {animal_data_dict["manure_type"]}')
                ...
                """logging.debug(f'Manures from {animal}: \n Manure solid: {manure_methane_dict["manure_solid"]}. '
                              f'\n Manure liquid: {manure_methane_dict["manure_liquid"]}'
                              f'\n Manure Straw: {manure_methane_dict["manure_straw"]}')
                logging.debug(f'Methane potential of {animal}:'
                              f'\n Methane solid: {manure_methane_dict["methane_solid"]}. '
                              f'\n Methane liquid: {manure_methane_dict["methane_liquid"]}'
                              f'\n Methane Straw: {manure_methane_dict["methane_straw"]}')"""
            ...
            # if the manure type doesn't exist for that animal, assume it's pig or poultry, which only have 1 manure type each and calculate with that
                ...
                #print(
                   # f'calculating manure and methane for {animal} with the else statement. Manure type: {animal_data_dict["manure_type"]}')
    ...
    """logging.debug(f'Methane potential in methane dict (Converted to m3):'
                  f'\n Methane values in methane dict: {methane_dict}')"""
    ...
    """logging.debug(f'Methane total in results dataframe:'
                  f'\n Methane tot: {dfc.find_value_in_results_df(results_df, "methane_tot")}.')"""


    # pre storage emissions ch4 in kg CH4, with emissions for solid, liquid and straw in m3
    ...
    nitrogen_emissions_dict = calc_farm_emissions_nitrogen(nitrogen_emissions_dict, env_config,
                                                           additional_data_dict)                # emissions in kg NH3 and kg N2O
    ...
    print(f'calculate single farm, nitrogen emission dict after adding to results: {nitrogen_emissions_dict}')
    ...
    print(f'Manure tot : {manure_tot}')
    print(f'Nitrogen emissions dict: {nitrogen_emissions_dict}')
    return results_df
...
def calc_all_farms_new(list_farms, animal_config, env_config):
    ...
    """logging.debug(f'Methane total and emissions in results dataframe after all farm calculations:'
                  f'\n Methane tot: {dfc.find_value_in_results_df(results_df, "methane_tot")}.'
                  f'\n Methane tot after emissions pre-storage: {dfc.find_value_in_results_df(results_df, "methane_tot_pre_storage")}'
                  f'\n Methane emissions pre-storage: {dfc.find_value_in_results_df(results_df, "methane_emissions_pre_storage")}')"""
    return results_df
...
def post_storage_and_field_emissions(input_df, env_config, ad):
    ...
    print(f'emissions dict in post_storage_and_field_emissions after calculating emissions: {emissions_dict}')

    """before storing, convert all emissions from kg C/N to kg CH4/NH3/N2O"""
    emissions_dict = convert_elemental_to_molecule_weight(emissions_dict, env_config)
    print(f'emissions dict in post_storage_and_field_emissions after converting emissions: {emissions_dict}')
    ...
    if not ad:
        ...
        list_nitrogen = ["co2_n2o_post_storage_untreated", "co2_n2o_field_untreated", "co2_n2o_tot_untreated",
                         'n2o_emissions_post_storage_untreated', 'n2o_emissions_field_untreated',
                         'n2o_emissions_untreated',
                         'nh3_emissions_post_storage_untreated', "nh3_emissions_field_untreated",
                         'nh3_emissions_untreated',
                         'co2_eq_tot_untreated']
        for key in list_nitrogen:
            new_key = key.replace('_untreated', '')
            dfc.store_value_in_results_df(input_df, new_key, 'value',
                                          dfc.find_value_in_results_df(input_df, key))

    else: #add methane pre-storage emissions to total emissions:
        methane_emissions_pre_solid = dfc.find_value_in_results_df(input_df, "methane_emissions_solid")
        methane_emissions_pre_liquid = dfc.find_value_in_results_df(input_df, "methane_emissions_liquid")
        methane_emissions_pre_straw = dfc.find_value_in_results_df(input_df, "methane_emissions_straw")
        methane_emissions_pre_tot = methane_emissions_pre_solid + methane_emissions_pre_straw + methane_emissions_pre_liquid
        dfc.add_value_to_results_df(input_df, "methane_emissions", "value", methane_emissions_pre_tot)
...
def calc_anaerobic_digestion(input_df, env_config):
    ...
    """logging.debug(f'Methane tot pre storage for AD calculations in m3: {methane_pot}'
                  f'\n Methane yield before losses in m3: {methane_yield}.'
                  f'\n Methane yield after losses in m3: {ad_results_dict["methane_yield"]}.'
                  f'\n Methane losses in m3: {ad_results_dict["methane_emissions_ad"]}')"""
    ...
    """calculate biogas composition and co2 content, after losses from AD in m3"""
...
def calc_chp_output(input_df, env_config):
    ...
    methane_to_chp = dfc.find_value_in_results_df(input_df, "methane_to_chp")       # in m3 methane
    heat, electricity = chp.energy_produced(methane_to_chp, env_config)                         # in kwh

    print(f'CHP methane, heat and electricity: {methane_to_chp}\n {heat} \n {electricity}')
    """store values in input df"""
    dfc.store_value_in_results_df(input_df, "heat_generated_chp", "value", heat)
    dfc.store_value_in_results_df(input_df, "electricity_generated_chp", "value", electricity)
    ...
    """logging.debug(f'Methane going to chp in m3: {methane_to_chp}'
                  f'\n Heat generated in CHP in kWh: {heat}.'
                  f'\n electricity generated in CHP in kWh: {electricity}.'
                  f'\n Total electricity generated stored in results dataframe: {dfc.find_value_in_results_df(input_df, "electricity_generated_tot")}')"""
...
def calc_biogas_upgrading(input_df, env_config):
    ...
    bg_results_dict['heat_generated_tot'], bg_results_dict['electricity_generated_chp'] = chp.energy_produced(
        bg_results_dict['methane_to_chp'], env_config)

    print(f'biogas results dict: {bg_results_dict}')

    """logging.debug(f'Calculating CHP and SOFC output:'
                  f'\n Methane going to chp in m3 (in results df): {bg_results_dict["methane_to_chp"]}.'
                  f'\n Heat generated in CHP in kWh: {bg_results_dict["heat_generated_tot"]}.'
                  f'\n electricity generated in CHP in kWh: {bg_results_dict["electricity_generated_chp"]}.'
                  f'\n electricity generated with SOFC: {bg_results_dict["electricity_generated_sofc"]}.'
                  f'\n Biomethane to SOFC for electricity generation: {bg_results_dict["biomethane_ch4"]}')"""
    ...
    """logging.debug(f'Calculating CHP output in last line of calc_biogas_upgrading function:'
                  f'\n Methane going to chp in m3 (in results df): {dfc.find_value_in_results_df(input_df, "methane_to_chp")}.'
                  f'\n Heat generated in CHP in kWh: {dfc.find_value_in_results_df(input_df, "heat_generated_chp")}.'
                  f'\n electricity generated in CHP in kWh: {dfc.find_value_in_results_df(input_df, "electricity_generated_chp")}.'
                  f'\n Total electricity generated stored in results dataframe: {dfc.find_value_in_results_df(input_df, "electricity_generated_tot")}.'
                  f'\n electricity generated with SOFC: {dfc.find_value_in_results_df(input_df, "electricity_generated_sofc")}.')"""
    return input_df
...
def calc_env_impacts(input_df, env_config):
    ...
    env_impact_dict['co2_eq_tot'] = (co2_eq_no_electricity + env_impact_dict['co2_electricity_demand_tot'] +
                                     env_impact_dict['co2_ad_construction']) + env_impact_dict['co2_chp_construction']
    ...
    electricity_gen_tot = dfc.find_value_in_results_df(input_df, 'electricity_generated_tot')
    heat_gen_tot = dfc.find_value_in_results_df(input_df, "heat_generated_tot")
    co2_eq_tot = env_impact_dict['co2_eq_tot']
    if electricity_gen_tot > 0:
        energy_gen_tot = electricity_gen_tot + heat_gen_tot
        co2_allocation_factor_el = electricity_gen_tot / energy_gen_tot             # allocating GWP100 according to energy
        co2_per_kwh_el = co2_allocation_factor_el * co2_eq_tot / electricity_gen_tot
        co2_per_kwh_energy = co2_eq_tot / (electricity_gen_tot + heat_gen_tot)


    else:
        co2_per_kwh_el = 0
        co2_per_kwh_energy = 0

    env_impact_dict['co2_per_kwh_el'] = co2_per_kwh_el
    env_impact_dict['co2_per_kwh_energy'] = co2_per_kwh_energy

    """Dalys from NH3 emissions and electricity mix for 'daly_nh3', 'daly_el_mix_consumption', 'daly_el_mix_production'"""
    nh3_emissions = dfc.find_value_in_results_df(input_df, "nh3_emissions")
    env_impact_dict['daly_nh3'] = envi.daly_nh3_emissions(env_config, nh3_emissions)

    el_consumption = dfc.find_value_in_results_df(input_df, "electricity_demand_tot")
    env_impact_dict['daly_el_mix_consumption'] = envi.daly_electricity_mix(env_config, el_consumption)

    el_production = dfc.find_value_in_results_df(input_df, "electricity_generated_tot")
    env_impact_dict['daly_el_mix_production'] = envi.daly_electricity_mix(env_config, el_production)

    """store impacts in input df"""
    store_dict_in_results(input_df, env_impact_dict)

    """logging.debug(f'environmental impact dict: {env_impact_dict}')"""
    methane_tot = dfc.find_value_in_results_df(input_df, "methane_tot")

    print(f'CO2 emissions tot: {co2_eq_tot} \n Electricity produced: {electricity_gen_tot} \n co2 per kwh: {co2_per_kwh_energy} \n methane tot: {methane_tot}')
