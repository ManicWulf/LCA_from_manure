...
calc_values_env_impact = ["co2_methane_pre_storage", "co2_methane_post_storage", "co2_methane_field", "co2_methane_ad", "co2_methane_biogas_upgrading",
                          ...
                          "co2_eq_tot", 'co2_eq_tot_untreated', 'co2_per_kwh_el', 'co2_per_kwh_energy',
                          'daly_nh3', 'daly_el_mix_consumption', 'daly_el_mix_production']
...
def check_combination_exists_in_animal_config(animal_type, stable_type, df):
    df = df.copy()  # Create a copy of the dataframe
    df['stable_type'] = df['stable_type'].astype(int)  # Convert stable_type to int

    # Apply string methods only to string values in animal_type column
    df['animal_type'] = df['animal_type'].apply(lambda x: x.strip().lower() if isinstance(x, str) else x)

    # Ensure input animal_type is a string and apply string methods
    animal_type = str(animal_type).strip().lower()
    stable_type = int(stable_type)  # Ensure input stable_type is an int

    # Print statements for debugging
    #print(f'During function to check: Animal type: {animal_type} and stable type: {stable_type}')
    #print("Dataframe contents for comparison:")
    #print(df[['animal_type', 'stable_type']])

    # Check if the combination exists
    return df[(df['animal_type'] == animal_type) & (df['stable_type'] == stable_type)].any().any()
