...
def nh3_emissions_manure(env_config, n_tot, application_method):
    ...
    return n_tot_new, nh3_emissions


def nh3_emissions_digestate(env_config, n_tot, application_method):
    factor_n_acc = get_acc_nitrogen_factor_digestate(env_config)
    n_acc = n_tot * factor_n_acc
    factor_field_nh3 = get_nh3_field_emissions_factor(env_config)
    application_factor = get_field_application_factor(env_config, application_method)
    nh3_emissions = n_acc * factor_field_nh3 * application_factor
    n_tot_new = n_tot - nh3_emissions
    return n_tot_new, nh3_emissions
...
def n2o_emissions(env_config, n_acc, nh3_emissions, application_method):
    ...
    """
    factor_n_acc = get_acc_nitrogen_factor_digestate(env_config)
    n_new = n_acc * factor_n_acc
    factor_field_n2o = get_n2o_field_emissions_factor(env_config)
    application_factor = get_field_application_factor(env_config, application_method)
    """
    ...
    """
    n2o_emission = n_new * factor_field_n2o * application_factor

    """
    debugging!
    """
    print(f'function n2o_emissions, field_application.py: \n'
          f'factor n accessible: {factor_n_acc} \n'
          f'n tot after nh3 emissions: {n_acc} \n'
          f'nh3 emissions: {nh3_emissions} \n'
          f'factor field n2o: {factor_field_n2o} \n'
          f'application factor: {application_factor} \n'
          f'n2o emissions: {n2o_emission} \n'
          f'end of n2o emissions, field application.py')
