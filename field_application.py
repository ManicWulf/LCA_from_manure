...
def get_field_application_factor(env_config):
    """get the field application factor, for reducing emissions based on application method"""
    return dfc.find_value_env_config("factor_field_application", "value", env_config)
...
def nh3_emissions_manure(env_config, n_tot):
    n_acc = n_tot * get_acc_nitrogen_factor_manure(env_config)
    nh3_emissions = n_acc * get_nh3_field_emissions_factor(env_config) * get_field_application_factor(env_config)
    return n_acc, nh3_emissions


def nh3_emissions_digestate(env_config, n_tot):
    n_acc = n_tot * get_acc_nitrogen_factor_digestate(env_config)
    nh3_emissions = n_acc * get_nh3_field_emissions_factor(env_config) * get_field_application_factor(env_config)
    return n_acc, nh3_emissions


def n2o_emissions(env_config, n_acc, nh3_emissions):
    """
    ...
    """
    n2o_emission = n_new * get_n2o_field_emissions_factor(env_config) * get_field_application_factor(env_config)
    return n2o_emission


def n_emissions_manure(env_config, n_tot):
    n_acc, nh3_emissions = nh3_emissions_manure(env_config, n_tot)
    n2o_emission = n2o_emissions(env_config, n_acc, nh3_emissions)
    return nh3_emissions, n2o_emission


def n_emissions_digestate(env_config, n_tot):
    n_acc, nh3_emissions = nh3_emissions_digestate(env_config, n_tot)
    n2o_emission = n2o_emissions(env_config, n_acc, nh3_emissions)
    return nh3_emissions, n2o_emission
